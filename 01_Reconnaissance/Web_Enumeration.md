...
**攻撃者の思考トレース:** Web は「バージョンが分かれば既知 CVE が最短経路」になることが多い。だからファジングより先に「何のアプリの何バージョンか」を手動 + whatweb で確定させ、CVE 検索を回す。ファジングは IP ブロックのリスクがあるため、防御告知を確認してから最後に回す。
...
| アプリ名は出るが**バージョンが取れない**（ログイン画面がバージョンを出さない製品は多い）| バージョンマッチ不能 | バージョンマッチを諦め、候補 PoC を `searchsploit -x` で読み「叩く URL/パラメータ」を把握 → そのエンドポイントの**存在（200/404）**と**二次シグナル**で適用可否を判定（下記「注意」）|
...
**バージョン文字列がどうしても取れないときは「バージョンマッチ」から「適用可否の直接判定」へ切り替える:**

- **エンドポイント存在判定**: 候補 PoC が叩く URL/パラメータを `searchsploit -x` で読み、その実在を `curl -skI [URL]` で確認。`200` ならその系統のコンポーネントが載っている＝適用見込み、`404` なら除外。バージョン文字列より確実な指標になることが多い
- **二次シグナルとの突き合わせ**: PoC の前提条件を、別途取れている事実と照合する。例として、PoC が PHP のヌルバイト切り詰め（`%00`）に依存するなら、ヘッダ等で取れた `X-Powered-By: PHP/[VER]` が **5.3.4 未満**かを見る（5.3.4 以降は `%00` がパスに使えず不発）。「バージョンそのもの」でなく「適用条件を満たす別の観測値」で当否を絞る
