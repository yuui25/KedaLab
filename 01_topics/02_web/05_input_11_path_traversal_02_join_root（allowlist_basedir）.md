# 05_input_11_path_traversal_02_join_root（allowlist_basedir）

## このファイルで扱う概念
- join/root設計とベースディレクトリ境界。
...
- 想定する環境（例：クラウド/オンプレ、CDN/WAF有無、SSO/MFA有無）：
  - ファイル参照機能（download/view/include/template/render/export/upload）を持つWebアプリ
  - 各種言語/フレームワークのjoin実装の違い
  - OS差（POSIX/Windows）によるパス解釈の違い
- できること/やらないこと（安全に検証する範囲）：
  - できること：join/rootの設計が崩れる条件を差分で確定する
  - やらないこと：破壊的なファイル操作、本番環境での過度な試行
- 依存する前提知識（必要最小限）：
  - `05_input_11_path_traversal_01_normalization（dotdot_encoding）.md`
- 扱う範囲（本ファイルの守備範囲）
  - 扱う：
    - join（base + user）と root（基準ディレクトリ）の扱い
    - allowlist（許可集合）の設計
    - base directory拘束の実装方式
    - symlink/hardlink/TOCTOUの問題
  - 扱わない（別ユニットへ接続）：
    - dotdot/encoding（表現揺れ） → `05_input_11_path_traversal_01_normalization（dotdot_encoding）.md`
    - アーカイブ展開時のjoin/root → `05_input_11_path_traversal_03_archive（zip_slip）.md`
    - 保存先のjoin/root → `05_input_12_file_upload_02_storage_path（bucket_acl_traversal）.md`
    - ファイル参照の認可モデル → `03_authz_08_file_access_ダウンロード認可（署名URL）.md`
...
PortSwigger等の一般的な防御指針は「検証後に base dir に結合し、FS APIで canonicalize、canonicalize後のパスが base dir から始まることを確認」と整理される。
...
- 取得する証跡（目的ベースで最小限）：
  - アプリログ（base_dir, candidate_path, resolved_path, decision, rule_id）
  - HTTPリクエスト/レスポンス（Proxyログ/HAR）
- 観測の取り方（どの視点で差分を見るか）：
  - 同一入力点で、相対想定と絶対想定を比較し、挙動が変わるかを見る
  - canonicalize→base拘束の強度を判定
- 実施方法（最高に具体的）：観測の準備と相関キー
  - 証跡ディレクトリ（必須）
    ~~~~
    mkdir -p ~/keda_evidence/path_traversal_join_root 2>/dev/null
    cd ~/keda_evidence/path_traversal_join_root
    ~~~~
  - 検証の前提を固定（スコープ事故を防ぐ）
    - 必須で決める（レポート先頭に書く）
      - 対象は **許可されたスコープ** のみ
      ...
      - 破壊的なファイル操作は行わない
  - 相関キー（最低限）を作る（後で必ず効く）
    - request_id、入力点、base_dir、candidate_path、resolved_path、decision、rule_id
...
- この例で観測していること：
  - join段からopen段までの各段階での値の変化を追跡し、どこで境界が崩れるかを特定する
- 出力のどこを見るか（注目点）：
  - candidate_path が base_dir を保持しているか（絶対パスでbaseが捨てられていないか）
  - resolved_path が base_dir 配下に収まっているか（canonicalize後の実体判定）
  - decision と rule_id（どの検証で拒否/許可されたか）
