# 05_input_04_nosql_injection_03_neo4j_cypher_03_authz（graph_traversal_scope）

## 危険性を一言で
- グラフ探索の範囲が広がると、本来見えないノード/関係が露出する。

## 最小限の成立判断（目安）
- 認可境界外のノードが見える差分が再現できる。

## 観測例（差分のイメージ）
- 期待: 自分のノードのみ、差分: 他ユーザーのノードが返る。
...
## 前提（対象・範囲・想定）
- 対象：関連ノード表示（related）、おすすめ（recommendation）、共通点、関係グラフ可視化、権限グラフ、監査グラフ、"パス（path）" "関係（relationship）" "深さ（depth）" の概念が出る機能
- 想定する環境（例：クラウド/オンプレ、CDN/WAF有無、SSO/MFA有無）：
  - アプリ権限：ユーザ→ロール→テナント/組織/プロジェクトなど、DB（Neo4j）権限：DBユーザ/ロール（これはアプリ権限と別物。混同が事故る）
  - 本ファイルの焦点：graph_traversal_scope：探索でスコープが漏れる設計問題（BOLA/BFLAのグラフ版）
  - 非スコープ：Cypher injection（構文注入）：別ファイル、APOC（外部到達/動的実行）：別ファイル
- できること/やらないこと（安全に検証する範囲）：
  - できること：スコープ（tenant/org/owner）の定義、探索（トラバーサル）での破れ方、返却縮退と可用性制限の設計、検証の取り方（差分＝成立根拠）に落として、実務で再現・評価・修正提案できる
  - やらないこと：大量抽出は避け、越境混入の成立根拠（1〜数件）を証拠化
- 依存する前提知識（必要最小限）：
...
- 何が"確定"できるか：
  - スコープ（tenant/org/owner）の定義、探索（トラバーサル）での破れ方、返却縮退と可用性制限の設計、検証の取り方（差分＝成立根拠）に落として、実務で再現・評価・修正提案できる
- 何が"推定"できるか（推定の根拠/前提）：
  - まず Boolean oracle（返却ノード数、関係数、件数、レスポンス長、"別tenant由来の識別子/名称"の混入（1件で成立根拠になる））で確定する（推奨）
  - エラーが見えるなら、エラーが詳細すぎる場合は情報露出として切り出す（oracle化）
  - 時間差分は最後（短時間・少回数で、他指標と組み合わせる）
- 何は"言えない"か（不足情報・観測限界）：
  - "探索範囲が確実に拡大した"の断定（仕様として高度検索が存在する可能性）、"DoSが可能"の断定（性能試験は別枠、契約と安全配慮が必要）
- よくある状態パターン（正常/異常/境界がズレている等）：
  - パターンA：探索で到達ノードにスコープ制約が適用されない（越境混入） → "到達したノードにもtenant条件を適用する" という当たり前が抜けがち
  - パターンB：探索ラベル/関係種別が入力で広がる（語彙の委譲） → 柔軟な検索要件のために「語彙（ラベル/関係）」をユーザに委譲している
  - パターンC：深さ（depth）や可変長パスが入力で増やせる（面積の爆発） → 深さ上限が仕様化されていない、または内部用の上限が外部に露出

## 攻撃者視点での利用（意思決定：優先度・攻め筋・次の仮説）
- この状態が示す"狙い目"：
  - 関連ノード表示（related）、おすすめ（recommendation）、共通点、関係グラフ可視化、権限グラフ、監査グラフ、"パス（path）" "関係（relationship）" "深さ（depth）" の概念が出る機能
- 優先度の付け方（時間制約がある場合の順序）：
  - まずテナントAのユーザ vs テナントBのユーザ、同一tenant内のロール差分（一般 vs 管理）で期待値を固定する、期待：越境しない、ロールに応じて許可範囲だけ変わる
- 代表的な攻め筋（この観測から自然に繋がるもの）：
  - 攻め筋1：related APIで越境する（入口2だけ漏れる） → related表示は別チーム/別実装で、tenant filterが抜けた
  - 攻め筋2：graph可視化でpathを丸返し（情報露出増幅） → フロント描画都合で内部表現を返した
- 「見える/見えない」による戦略変更（例：CDN配下、SSO前提、外部委託先など）：
  - IDOR/BOLA と異なる「探索で越境」する形（graph traversal）をテスト観点に入れる、入力が探索深さや探索対象ラベルを広げられる場合、Injectionでなくても越境の成立点になる
...
- この例で観測していること：スコープ制約の適用範囲、探索語彙のallowlist、深さ/件数の上限、返却縮退の設計
- 出力のどこを見るか（注目点）：返却ノード数、関係数、件数、レスポンス長、"別tenant由来の識別子/名称"の混入（1件で成立根拠になる）
- この例が使えないケース（前提が崩れるケース）：グラフ探索がそもそも使用されていない場合、または完全に静的なクエリのみを使用している場合
...
- WSTG：
  - 該当カテゴリ/テスト観点：WSTG-AUTHZ-01 Authorization Testing、WSTG-INPV-01 Input Validation
...
- PTES：
  ...
  - 前後フェーズとの繋がり（1行）：情報収集→脆弱性分析：グラフ機能（関連/おすすめ/可視化/監査）を入口に、スコープの破れを差分で確定し、侵入→影響実証：大量抽出は避け、越境混入の成立根拠（1〜数件）を証拠化、報告：原因を「スコープ固定不備」「返却縮退不足」「探索制限不備」「権限モデル混同」に分解して提案
- MITRE ATT&CK：
  - 該当戦術（必要なら技術）：TA0009 Collection（グラフ探索は関係性情報の収集に強い）、TA0007 Discovery（内部関係の発見（組織/権限/関係）に繋がる）
  - 攻撃者の目的（この技術が支える意図）：入口は公開アプリならT1190に接続
