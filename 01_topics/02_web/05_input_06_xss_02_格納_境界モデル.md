...
## このファイルで扱う概念
- 格納XSS：保存された入力が他ユーザーの表示時に実行される。

## 危険性を一言で
- 他者閲覧時に自動実行され、影響範囲が拡大する。

## 最小限の成立判断（目安）
- 保存→表示の経路で A/B 差分が再現し、実行点が特定できる。

## 観測例（差分のイメージ）
- A: 保存後も文字列表示、B: 表示時にHTMLとして解釈される。

## 観測が取れない場合の代替
- 保存データの正規化/サニタイズ有無をストレージ/ログで確認する。

## 時間制約下の最小観測点
- 保存後の表示における差分と、影響ユーザー範囲の把握。
