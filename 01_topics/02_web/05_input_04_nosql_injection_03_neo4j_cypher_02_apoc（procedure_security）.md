...
## 前提（対象・範囲・想定）
- 対象："インポート/エクスポート/同期" "外部データ参照" "CSV/JSONロード" "データ加工" "ETL" "可視化" 等の機能、管理機能（データ管理/バッチ処理）に隠れていることが多い
- 想定する環境（例：クラウド/オンプレ、CDN/WAF有無、SSO/MFA有無）：
  - アプリ→Neo4j（Bolt/HTTP）、APOC有効（Community/Enterprise、構成で差がある）、プロシージャ呼び出しはCypher経由で行われる（CALL dbms.procedures / CALL apoc.*）
  - 本ファイルの焦点：APOCによって "NoSQLi（Cypher Injection）" が **別の影響面（外部到達・動的実行）** に拡張される点
  - 注意（実務）：APOCの具体的な悪用手順・payload提示は避け、観測設計と防御設計に寄せる、ラボで再現する場合も、外部到達は自分の管理下（ローカル・ダミー）に限定し、実環境での到達テストは抑制する
- できること/やらないこと（安全に検証する範囲）：
  - できること：アプリがAPOCを使っている入口（機能）を同定できる、露出しているAPOCの種類を「危険系（外部到達/動的実行/ファイル）」として分類できる、成立根拠（差分＝oracle）を低侵襲に確定できる、修正方針を「APOC禁止」ではなく **allowlist + 権限分離 + 引数型拘束 + 設定** に落とせる
  - やらないこと：影響実証は最小限（到達性/越境混入/情報露出の根拠まで）。外部到達や高負荷は避ける
- 依存する前提知識（必要最小限）：
  ...
  - `01_topics/02_web/09_ssrf_01_reachability（internal_localhost_metadata）.md`
...
- 何が"確定"できるか：
  - アプリがAPOCを使っている入口（機能）を同定できる、露出しているAPOCの種類を「危険系（外部到達/動的実行/ファイル）」として分類できる、成立根拠（差分＝oracle）を低侵襲に確定できる、修正方針を「APOC禁止」ではなく **allowlist + 権限分離 + 引数型拘束 + 設定** に落とせる
- 何が"推定"できるか（推定の根拠/前提）：
  - まず Boolean oracle（成功/失敗、件数/反映結果）で確定する（推奨）
  - エラーが見えるなら、"procedure not found / not allowed / security exception" 等の差分（ただし詳細エラーを返す場合は情報露出として独立に指摘）
  - 時間差分は最後（短時間・少回数で、他指標と組み合わせる）
- 何は"言えない"か（不足情報・観測限界）：
  - "外部到達が確実に可能"の断定（環境/設定で差が大きい）、"DoSが可能"の断定（性能試験は別枠、契約と安全配慮が必要）
- よくある状態パターン（正常/異常/境界がズレている等）：
  - パターンA：外部到達（SSRF類似の境界） → URL/HTTP経由でデータ取得・変換する系、入力がURLに入ると、アプリ→DB→外部/内部 という到達性が開く可能性
  - パターンB：動的Cypher実行（Injection増幅器） → 文字列からCypherを実行する/構文生成する系、Cypher Injectionが成立した場合の影響が"検索条件改変"から"任意クエリ実行"寄りに増幅する
  - パターンC：ファイル/ローカル資源アクセス（環境依存の高影響） → file:// の読み込み、エクスポート先指定など、DBが動くホストの資源へ触れる可能性（ただし環境/設定で差が大きい）

## 攻撃者視点での利用（意思決定：優先度・攻め筋・次の仮説）
- この状態が示す"狙い目"：
  - "インポート/エクスポート/同期" "外部データ参照" "CSV/JSONロード" "データ加工" "ETL" "可視化" 等の機能、管理機能（データ管理/バッチ処理）に隠れていることが多い
- 優先度の付け方（時間制約がある場合の順序）：
  - まずある機能操作で "procedure not allowed / not found" の差分が出る、または成功痕跡が出るかを確認、その機能がどのapocカテゴリ（外部到達/動的実行/ファイル/高コスト）に属するかを分類し、優先度を付ける
- 代表的な攻め筋（この観測から自然に繋がるもの）：
  - 攻め筋1：APOC呼び出し自体が可能（層1が成立） → ある機能操作で "procedure not allowed / not found" の差分が出る、または成功痕跡が出る
  - 攻め筋2：入力が引数に到達し、到達性/実行が入力依存で揺れる（層2が成立） → URL/クエリ/パス/パラメータの形で結果（成功/失敗/件数）が再現性を持って変わる
- 「見える/見えない」による戦略変更（例：CDN配下、SSO前提、外部委託先など）：
  - Cypher Injectionの延長として、プロシージャ呼び出しが「実行境界（機能呼び出し）」になっていないかを検証、APOCの入出力機能は、入力→外部到達（URL）/ローカル資源（file）へ繋がるため、WSTGの該当観点へ接続する
...
- この例で観測していること：プロシージャ名が固定で、入力はparams（data）に閉じる設計
- 出力のどこを見るか（注目点）：プロシージャの実行可否、引数の型拘束、エラーハンドリング
- この例が使えないケース（前提が崩れるケース）：プロシージャがそもそも使用されていない場合、または完全に静的なクエリのみを使用している場合
...
- 出力のどこを見るか（注目点）：入力の形で結果（成功/失敗/件数）が再現性を持って変わる、URL/クエリ/パス/パラメータの形で結果が揺れる
- この例が使えないケース（前提が崩れるケース）：プロシージャがそもそも使用されていない場合、または完全に静的なクエリのみを使用している場合
...
- PTES：
  ...
  - 前後フェーズとの繋がり（1行）：APOC利用箇所（CSVインポート、外部参照、動的Cypher、変換）を特定し、入力点→引数→効果（到達性/実行）の差分で成立根拠を取る、影響実証は最小限（到達性/越境混入/情報露出の根拠まで）。外部到達や高負荷は避ける、原因を「APOC露出」「許可プロシージャ過多」「引数検証不足」「権限伝播不備」「エラーモデル不備」「運用設定不備」に分解して提示
- MITRE ATT&CK：
  - 該当戦術（必要なら技術）：TA0001 Initial Access / TA0009 Collection / TA0008 Lateral Movement（外部到達や内部資源アクセスがあれば）/ TA0005 Defense Evasion
  - 攻撃者の目的（この技術が支える意図）：T1190（公開アプリ入口）、SSRF的な到達があれば内部探索へ波及
