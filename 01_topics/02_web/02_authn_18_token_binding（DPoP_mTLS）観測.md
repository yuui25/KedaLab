...
### 2.1) 簡易フロー（AS/RS）
~~~~
Client -> AS(/token): DPoPヘッダ or mTLS で発行を拘束
Client -> RS(/api): DPoPヘッダ or クライアント証明書が必須
ASで拘束しても、RSで強制しないと成立しない
~~~~
...
- DPoPヘッダ（署名JWT）の主要構成（観測で意味がある部分）
 ...
 - 観測の具体例（値は伏せる）
   - htm が `GET`、htu が実際のAPI URL と一致しているか
   - jti が毎回変わるか（再送で同一になっていないか）
