...
## 危険性を一言で
- 入力がオブジェクト構築やメソッド実行に到達すると、型やガジェット経由で実行境界を越える。

## 最小限の成立判断（目安）
- 想定型と異なる入力で挙動が変わり、型制約で差分が消えることを確認する。
- 「ガジェット前提」は、既存の到達経路があるかを根拠として示す。

## 観測例（差分のイメージ）
- A: 想定型で正常処理、B: 想定外型で例外/副作用が発生する。

## 対策の優先順位
1) デシリアライズ回避（形式変更・手動パース）
2) 型/クラスのallowlist固定
3) 署名検証・完全性チェック
