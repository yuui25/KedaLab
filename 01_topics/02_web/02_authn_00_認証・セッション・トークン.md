...
## 推奨読書順序（最小）
1) `01_topics/02_web/01_web_00_recon_入口・境界・攻め筋の確定.md`
2) `01_topics/02_web/02_authn_01_cookie属性と境界（Secure_HttpOnly_SameSite_Path_Domain）.md`
3) `01_topics/02_web/02_authn_02_session_lifecycle（更新_失効_固定化_ローテーション）.md`
4) `01_topics/02_web/02_authn_03_token設計（Bearer_JWT_Refresh_Rotation）.md`
...
## 優先度基準（P0/P1/P2）
- P0：本人性の成立点が不明で、以後の検証が成立しない状態
- P1：成立点は見えるが、寿命/例外/回復が不明で影響が読めない状態
- P2：成立点と寿命が見えており、詳細設計の確認に移れる状態
