# 04_api_08_file_export_エクスポート境界（CSV_PDF）

## 目的（この技術で到達する状態）
- CSV/PDF等のエクスポートを「最大の漏えい出口」として扱い、(1)生成対象範囲の境界（tenant/role/state/filters）、(2)生成物参照の境界（IDOR/共有/期限/署名）、(3)データ最小化（列/マスキング/集計化）、(4)出力安全性（CSV Injection/テンプレ注入/HTML→PDF）、(5)保存と配布（S3/署名URL/メール添付）、(6)非同期・再生成・キャッシュ混線、(7)監査と濫用耐性、をモデル化して検証者が低侵襲で確定できる
...
- 想定する環境（例：クラウド/オンプレ、CDN/WAF有無、SSO/MFA有無）：
  - 典型の現実構成：export request（条件・列・期間）→ job_id → artifact_id → download_url（署名URL）、生成物は一時保存（期限付き）だが、ログ/添付で長期化することがある
  - 重要前提：エクスポートは"データの再構成"であり、一覧APIとは別クエリ/別権限で実装されがち（＝穴）、非同期は"時間差"があるため、権限変更/状態変更後に生成が走る（07の再評価が必須）
  - エクスポートは「一覧/検索（03）」と「非同期（07）」の出口であり、実装の例外パスが最も多い。生成物は"ファイル/URL/キャッシュ"として残るため、漏えいは長期化しやすい。
- できること/やらないこと（安全に検証する範囲）：
  - できること：2ユーザ×（可能なら）2テナントで差分を取る、大量生成は不要。少量データで境界を示す、同じ条件をUIとAPIで比較する、相関キーを固定し控える、証跡は「生成物そのもの」ではなく「参照可否」と「含まれる範囲（最小サンプル）」で取る
  - やらないこと：生成物を丸ごと保存/共有せず、機微は最小限マスクして証跡化（実務配慮）
- 依存する前提知識（必要最小限）：
...
- 何が"推定"できるか（推定の根拠/前提）：
  ...
  - export条件が自由・上限無しの場合、外部検証でも"低工数大量漏えい"に直結
- 何は"言えない"か（不足情報・観測限界）：
  - 保存先（S3等）の設定詳細。だが、URL形状と期限挙動から安全性を評価できる。
- よくある状態パターン（正常/異常/境界がズレている等）：
  - パターンA：生成範囲（filters/columns）がサーバで強制されず、越境混入が起きる → 越境混入（P0）
  - パターンB：生成物参照がIDOR/越境になっている → 参照成功（P0）
  - パターンC：署名URLの束縛・期限・失効が弱い → 長寿命・束縛弱い（P0〜P1）
  - パターンD：CSV/PDF特有の出力リスク（CSV Injection、テンプレ境界）が抑えられていない → 出力リスク（P1〜P0、内容次第）
  - パターンE：濫用耐性（巨大生成/再生成/並列）と監査が整備されていない → 濫用耐性不足（P1）

## 確認（最大限詳しく：どうやって“確定”したと言えるか）
エクスポートは「入口→生成→出口」の3点セットで確認する。特に出口（artifact参照）の越境は、少数リクエストで確定でき、検証報告に強い。
...
- ASVS：
  ...
  - このファイルの内容が「満たす/破れる」ポイント：アクセス制御（生成物の参照/共有/期限）、最小開示（PII・機微列の制御）、入力検証（エクスポート条件・テンプレ）、出力エンコーディング（CSV Injection等）、機密データ保護（保存先・暗号化・署名URL）、監査（誰が何を出したか）、可用性（巨大生成・ワーカー枯渇）
- WSTG：
  - 該当カテゴリ/テスト観点：Authorization Testing（生成物IDOR、共有境界）、API Testing（エクスポート条件、署名URL、期限、キャッシュ）、Business Logic（フィルタ/状態/権限の反映）、Input/Output Handling（CSV Injection、PDF生成のテンプレ境界）、Error Handling（エラー差分/情報漏えい）
  - 該当が薄い場合：この技術が支える前提（情報収集/境界特定/到達性推定 等）：エクスポートの"入口（条件）→生成（ジョブ）→出口（ファイル参照）"を分解し、(1)越境混入、(2)生成物参照のIDOR、(3)署名URLの束縛/期限、(4)出力安全性（CSV/PDF）、(5)ログ/監査、を低侵襲で確定する
- PTES：
...
- MITRE ATT&CK：
  - 該当戦術（必要なら技術）：TA0009（Collection）、TA0010（Exfiltration）、TA0040（Impact）
  - 攻撃者の目的（この技術が支える意図）：エクスポート機能で大量取得（Collection）、生成物URLや添付で持ち出し（Exfiltration）、巨大生成・無限再生成でワーカー枯渇（Impact）。
