# 05_input_10_open_redirect（遷移先信頼境界）

## このファイルで扱う概念
- 遷移先の信頼境界と認可フローへの影響。

## 危険性を一言で
- 認証/認可フローが外部に誘導され、トークンや意図が奪われる。

## 最小限の成立判断（目安）
- 遷移先の差分が再現し、制限が回避できる。

## 観測例（差分のイメージ）
- A: 許可ドメインのみ、B: 任意ドメインへ遷移。

## 観測が取れない場合の代替
- redirect_uriの検証条件と正規化処理を確認する。
...
- 想定する環境（例：クラウド/オンプレ、CDN/WAF有無、SSO/MFA有無）：
  - リダイレクト機能を持つWebアプリ（ログイン/ログアウト、パスワードリセット、決済、招待等）
  - 認証フロー（SSO/OAuth/OIDC）が絡む可能性
- できること/やらないこと（安全に検証する範囲）：
  - できること：遷移先がユーザ入力で決まるかを成立根拠（差分）で確定する
  - やらないこと：外部ドメインに"実害のあるURL"を置かない、破壊的な操作は行わない
- 依存する前提知識（必要最小限）：
  - `01_topics/02_web/01_web_00_recon_入口・境界・攻め筋の確定.md`
- 扱う範囲（本ファイルの守備範囲）
  - 扱う：
    - Open Redirect（CWE-601）：リダイレクト先URL（またはそれに等価な値）をユーザ入力として受け取り、十分な検証なしに遷移させる問題
    - 遷移の実装形態（サーバサイド：HTTP 3xx + Location、クライアントサイド：window.location等、HTML：meta refresh等）
  - 扱わない（別ユニットへ接続）：
    - 認可境界（forward系の亜種） → `03_authz_*`
    - SSRF詳細 → `05_input_09_ssrf_*`
...
- サーバサイド：
  - HTTP 3xx + Locationヘッダ
  - Refreshヘッダ、HTML meta refresh
- クライアントサイド：
  - window.location 操作、router push、アンカーのhref生成
- ログ：
  - どのパラメータで遷移先が決まったか（next/url等）
  - 判定結果（allowlist hit / warn / block）
  - trace_id（後追いできるように）
...
### 何が"確定"できるか：
- 遷移先がユーザ入力で決まるか（Open Redirectの成立）
- 外部へ自動遷移できるか（警告/中止の有無）
- 制約がある場合の回避可能性（正規化/パース差分）

### 何が"推定"できるか（推定の根拠/前提）：
- 制約の種類（相対のみ/allowlist/プレフィックス/正規表現）
- 影響範囲（フィッシング止まりか、認証・トークン・権限回りに波及するか）

### 何は"言えない"か（不足情報・観測限界）：
- 具体的な攻撃手順（本プロジェクトでは"成立条件"と"境界"が主）
- 影響の確定（認証フローとの結び付きの詳細確認が必要）

## 攻撃者視点での利用（意思決定：優先度・攻め筋・次の仮説）
- フィッシング/信用悪用（基本）：正規ドメインで誘導できるので、被害者は警戒しにくい
- 認証フロー（SSO/OAuth/OIDC）と結び付くと重くなる：認証コード/トークンがURLに載る、Referer漏えい、自動遷移
- アクセス制御回避（forward系の亜種）：認可チェックの境界を飛び越える可能性
- 他脆弱性の踏み台（連鎖）：SSRFのallowlist回避、WAF/Link scannerの挙動差

## 次に試すこと（仮説A/Bの分岐と検証）
### 仮説A：制約が強く、回避が困難
- 観測：
  - allowlist（完全一致）で外部ホスト/パスを限定している
  - 中間ページ（警告＋遷移先表示＋キャンセル）がある
- 次の一手：
  - 認証フローとの結び付きを評価
  - 他脆弱性との連鎖可能性を評価
...
### 仮説B：制約が弱く、回避が可能
- 観測：
  - プレフィックス/startsWith系、正規表現、文字列判定のみ
  - 正規化/パース差分で回避できる
- 次の一手：
  - 回避パターンを立証
  - 影響の最大化（現実導線に寄せる）
...
- 検証環境（関連する `04_labs/`）
  - 追加候補Lab（例）
    - `04_labs/02_web/05_input/10_open_redirect/`
- 取得する証跡（目的ベースで最小限）：
  - HTTPリクエスト/レスポンス（Proxyログ/HAR）
  - アプリログ（遷移先パラメータ、判定結果、trace_id）
- 観測の取り方（どの視点で差分を見るか）：
  - 同一注入点で入力だけを変え、Location/JSの差分を証拠化
  - 相対パス、絶対URL、変形URLで挙動を比較
- 実施方法（最高に具体的）：観測の準備と相関キー
  - 証跡ディレクトリ（必須）
    ~~~~
    mkdir -p ~/keda_evidence/open_redirect 2>/dev/null
    cd ~/keda_evidence/open_redirect
    ~~~~
  - 検証の前提を固定（スコープ事故を防ぐ）
    - 必須で決める（レポート先頭に書く）
      - 対象は **許可されたスコープ** のみ
      - 外部ドメインに"実害のあるURL"を置かない
      - 代表注入点×代表回避パターンの少数で確定する
  - 相関キー（最低限）を作る（後で必ず効く）
    - request_id、遷移先パラメータ、判定結果、trace_id
...
- この例で観測していること：
  - 入力→Locationの直結が強く疑われることを確認
- 出力のどこを見るか（注目点）：
  - Locationヘッダが外部になっている（サーバサイド）
  - JSが外部へ遷移している（client-side）
  - 中間ページ/警告が出るか（"仕様として外部遷移する"場合の必須要件）
- この例が使えないケース（前提が崩れるケース）：
  - 制約がある場合（allowlist/プレフィックス/正規表現）は、追加の回避パターン確認が必要
...
## リポジトリ内リンク（最大3つまで）
- 関連 topics：`02_authn_04_sso_oidc_flow観測（state_nonce_code_PKCE）.md`
- 関連 topics：`03_authz_00_認可（IDOR BOLA BFLA）境界モデル化.md`
