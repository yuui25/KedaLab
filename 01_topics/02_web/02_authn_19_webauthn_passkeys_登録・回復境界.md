...
### 2.1) WebAuthnの最小構造（観測に必要な部品）
- clientDataJSON（type/challenge/origin）: challengeが紐付いているかの根拠
- rpId: どのドメインに結びつくか（サブドメイン差に注意）
- attestationObject（登録）: 受理されたcredentialの種類/形式の手掛かり
- authenticatorData（認証）: user verification / signCount の挙動
- 署名: サーバ側で検証される前提（失敗時のエラーが証拠になる）
