...
- この例で観測していること：
  - CORSは「通常リクエスト」だけ見ても不十分（OPTIONS（プリフライト）を観測、401/403/500/302の例外でも同じヘッダが付くか確認、Max-Age（反映遅延）と Vary: Origin（混線防止）を要点として記録）
- 出力のどこを見るか（注目点）：
  - OPTIONS（プリフライト）、401/403/500/302の例外、Max-Age（反映遅延）、Vary: Origin（混線防止）
- この例が使えないケース（前提が崩れるケース）：
  - CORSが設定されていない場合、観測できない
