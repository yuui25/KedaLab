...
## 役割（00と01の分担）
- 00：全体像と用語、境界の考え方を固定する
- 01：主体/対象/操作/条件の具体的なモデル化と差分観測の設計に落とす

## 推奨読書順序（最小）
1) `01_topics/02_web/02_authn_00_認証・セッション・トークン.md`
2) `01_topics/02_web/03_authz_00_認可（IDOR BOLA BFLA）境界モデル化.md`
3) `01_topics/02_web/03_authz_01_境界モデル（オブジェクト_ロール_テナント）.md`
4) `01_topics/02_web/03_authz_02_idor_典型パターン（一覧_検索_参照キー）.md`
...
## 優先度基準（P0/P1/P2）
- P0：テナント境界が崩れる/不明で、影響が最大化する状態
- P1：ロール境界が崩れる/例外パスが残る状態
- P2：オブジェクト境界の揺れが残る状態
...
## 監査ログの確認方法（最小）
- どこを見るか：管理画面の監査/操作ログ、API側のアクセスログ
- 記録すべき項目：主体（user/role/tenant）、対象（object id）、操作（read/write）、結果（status）
- 使い方：A/B差分の結果と突き合わせて「どの境界で拒否されたか」を説明する
