...
### 5.1) 監査ログの確認方法（最小）
- どこを見るか（代表）
  - アプリ監査ログ（DBのaudit_events、SIEM、監査UI）
  - APIログ（admin-apiのaccessログ、decisionログ）
  - IdPログ（管理者の認証イベント、step-upの履歴）
- 最小の確認手順
  1. 代表操作（search / grant-role / export）のrequest_idを控える
  2. 監査ログで同一request_idを検索し、actor/tenant/action/target/reasonが揃っているか確認
  3. deny操作もログに残るか確認（拒否の証跡）
- ログ例（抜粋）
~~~~
{ "request_id":"req-123", "admin_actor":"A-01", "admin_role":"support",
  "tenant_id":"T1", "action":"grant-role", "target":"U123", "decision":"deny",
