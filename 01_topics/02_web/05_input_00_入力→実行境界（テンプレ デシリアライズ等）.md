# 05_input_00_入力→実行境界（テンプレ デシリアライズ等）

## 危険性を一言で
- 入力が「値」ではなく「構文/コード」として解釈されると、境界の外側で意図しない実行が起きる。

## 最小限の成立判断（目安）
- 入力点→解釈点→実行点のつながりが、A/B比較で安定して差分として観測できる。
- エラーは「成立根拠」と「情報露出」を分離して記録し、根拠は差分の再現性で示す。

## 観測例（差分のイメージ）
- Boolean: 件数が 1 件→0 件、あるいは 0 件→複数件に変わる。
- Error/Time: 4xx/5xx の有無が変わる、または応答が +3s 以上遅延する。

## 対策の優先順位
1) 実行経路の排除（機能/経路の再設計）
2) 構造拘束（プレースホルダ/allowlist/型制約）
3) 監視と証跡（差分検知・ログ・再現性の確保）
...
- 想定する環境（例：クラウド/オンプレ、CDN/WAF有無、SSO/MFA有無）：
  ...
  - 設定が到達性を変えることがある（CORS/ヘッダ/Secrets/連携設定）
- できること/やらないこと（安全に検証する範囲）：
  - できること：入力点→解釈点→実行点の"到達性"を観測し、検証の当たり所を自分で決められる
  - やらないこと：ペイロード集の羅列（「これ入れると出ます」だけは不可）、"落とし穴"章は作らない、具体的な脆弱性の断定（成立条件の追加観測が必要）、内部実装の確定（ソース/ログがないと推定に留まる）、影響の最大化（許可範囲と安全性の制約がある）
- 依存する前提知識（必要最小限）：
...
- よくある状態パターン（正常/異常/境界がズレている等）：
  - パターンA：入力がテンプレに入っている（通知/帳票/画面生成） → 解釈点（テンプレの存在）が"状態"で説明できる
  - パターンB：入力がクエリ生成に入っている（検索/フィルタ/レポート） → "どの条件で挙動が変わるか"が説明でき、解釈点（SQL/NoSQL/検索）を絞れる
  - パターンC：入力が外部呼び出し（SSRF相当）に繋がっている → 外部呼び出しの"到達性"と"成立条件"が説明できる
...
- ASVS：
  - 該当領域/章：V5（入力検証）
  - 該当要件（可能ならID）：V5.1（入力検証）
  - このファイルの内容が「満たす/破れる」ポイント：入力の扱い（検証/正規化/エンコード/実行境界）を"境界"として整理し、満たす/破れる点を観測で説明できる状態にする。入力が「どこまで到達し」「どこで解釈/実行に変わるか」を境界として扱う。
- WSTG：
  - 該当カテゴリ/テスト観点：Input Validation Testing、Injection Testing
  - 該当が薄い場合：この技術が支える前提（情報収集/境界特定/到達性推定 等）：Input Validation / Injection 系の観点を、入力点→解釈点→実行点の"到達性"として整理し、検証順を作る。このファイルは「他の脆弱性検証の前提（入力がどこへ届くか）」を支える。
- PTES：
  - 該当フェーズ：Information Gathering、Vulnerability Analysis、Exploitation
  - 前後フェーズとの繋がり（1行）：Vulnerability Analysis で"実行境界"に到達する入力を最短で特定し、Exploitation の成立条件を明確化する。実行境界に近い入力を絞り、検証優先度を作る。
- MITRE ATT&CK：
  - 該当戦術（必要なら技術）：TA0002（Execution）、TA0003（Persistence）、TA0007（Discovery）
  - 攻撃者の目的（この技術が支える意図）：Execution / Persistence 等に直結し得る「入力→実行」導線を、攻撃者の目的と境界で説明できる状態にする。技術名の貼り付けではなく「攻撃者の目的に対して、どの入力が実行境界に繋がるか」を説明できる状態にする。
