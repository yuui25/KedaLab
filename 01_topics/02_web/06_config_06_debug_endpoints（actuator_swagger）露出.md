...
- この例で観測していること：
  - 露出の分類（情報/仕様/操作）と境界（CDN/WAF経由と直オリジン（可能なら）で差分、UIだけ認証/ APIは無認証 の分断がないか）
- 出力のどこを見るか（注目点）：
  - 情報（読むだけ）、仕様（攻撃面の地図）、操作（状態変更）、CDN/WAF経由と直オリジン（可能なら）で差分、UIだけ認証/ APIは無認証 の分断
- この例が使えないケース（前提が崩れるケース）：
  - デバッグ/管理/仕様公開エンドポイントが存在しない場合、観測できない
