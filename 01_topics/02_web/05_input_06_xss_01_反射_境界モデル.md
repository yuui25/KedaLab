...
## このファイルで扱う概念
- 反射XSS：入力が即時にHTML/JSへ反映され、表示時に実行される。

## 危険性を一言で
- 出力がスクリプトとして解釈されると、閲覧者の操作やセッションが乗っ取られる。

## 最小限の成立判断（目安）
- 同一条件で A/B 差分が再現し、出力コンテキストが特定できる。

## 観測例（差分のイメージ）
- A: 文字列として表示、B: HTML/JSとして解釈され表示が変わる。

## 観測が取れない場合の代替
- 出力HTMLのエスケープ有無、CSP/テンプレ設定をログ/レスポンスから確認する。

## 時間制約下の最小観測点
- 反映位置の特定（HTML/属性/JS/URL）と、A/Bの表示差分の再現。
