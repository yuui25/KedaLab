...
- この例で観測していること：
  - ログを見るときの観点（秘密/PIIが混入していないか（URL/ヘッダ/例外）、相関できるか（request_id/trace_id）、構造が壊れないか（改行/制御文字/JSON崩壊））
- 出力のどこを見るか（注目点）：
  - 秘密/PIIが混入していないか（URL/ヘッダ/例外）、相関できるか（request_id/trace_id）、構造が壊れないか（改行/制御文字/JSON崩壊）
