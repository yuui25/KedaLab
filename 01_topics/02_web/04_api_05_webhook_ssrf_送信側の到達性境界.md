# 04_api_05_webhook_ssrf_送信側の到達性境界
...
- 検証として、危険な到達（内部IP/メタデータ）を“実害を出さずに”成立条件として示し、是正（設計/実装/ネットワーク）に落とせる
...
- 想定する環境（例：クラウド/オンプレ、CDN/WAF有無、SSO/MFA有無）：
  - 典型構成（現実的）：管理UI/APIでURL登録 → イベント発火でサーバがHTTP送信、非同期ジョブ（キュー/ワーカー）から送信される（07）、再送（失敗時リトライ）がある（04/06）、送信ログ（成功/失敗、ステータス、レスポンス断片）が管理画面で見られる場合がある（情報漏洩に接続）
  - 重要な前提：SSRFは「内部到達」だけでなく「外部向けの権限濫用（プロキシ化）」「情報収集」「DoS」も含む、クラウド環境ではメタデータサービス（IMDS等）が最重要到達先になり得る（ただし実行を煽らず"境界設計"として扱う）
  - Webhook送信は「サーバが外へHTTPを出す」機能であり、送信先がユーザ入力になるとSSRF（内部到達/メタデータ/横展開）の典型入口になる。さらに"再送"や"非同期処理"と組み合わさると、検知・停止が遅れやすい。
- できること/やらないこと（安全に検証する範囲）：
  - できること：送信先登録→送信実行（イベント発火）という2段の設計を、「URL正規化」「allowlist」「DNS解決」「リダイレクト」「プロキシ/解決経路」「レスポンス取り扱い（保存/表示）」で分解し、到達性境界が成立しているかを低侵襲に確定する
  - やらないこと：内部ネットワークの全到達範囲（環境依存）。ただし"privateレンジ拒否/egress制御"の有無は境界として評価できる。
- 依存する前提知識（必要最小限）：
...
- よくある状態パターン（正常/異常/境界がズレている等）：
  - パターンA：allowlist/正規化が弱い → 危険形状が登録可能（P0〜P1）
  - パターンB：リダイレクト追従で迂回できる → 追従＋未検証（P0）
  - パターンC：DNS変動で迂回できる（送信時検証が無い） → 送信時検証無し（P1、環境次第でP0）
  - パターンD：レスポンス持ち帰りが可能（漏洩経路） → 本文表示＋権限広い（P0〜P1）
  - パターンE：リトライが増殖し、停止が難しい → unsafe retry（P1、可用性/費用リスク）
...
{ "error": "url_not_allowed", "reason": "private_range" }
~~~~
- この例で観測していること：
...
- ASVS：
  - 該当領域/章：V5（入力検証）、V13（API）
  - 該当要件（可能ならID）：V5.1（入力検証）、V13.1（APIの認証・認可）、V13.2（API濫用耐性）
  - このファイルの内容が「満たす/破れる」ポイント：外部通信の信頼境界（送信先URLの制御）、SSRF対策（到達性の制限・正規化・リダイレクト制御）、認証情報の保護（メタデータ/内部サービスへのアクセス防止）、監査（送信先と結果）、濫用耐性（連打・長時間接続・巨大レスポンス）
- WSTG：
  - 該当カテゴリ/テスト観点：Server-Side Request Forgery（SSRF）、API Testing（送信先URL、リダイレクト、DNS、エラー差分）、Authorization Testing（誰が送信先を登録/変更できるか：運用権限境界）
  - 該当が薄い場合：この技術が支える前提（情報収集/境界特定/到達性推定 等）：送信先登録→送信実行（イベント発火）という2段の設計を、「URL正規化」「allowlist」「DNS解決」「リダイレクト」「プロキシ/解決経路」「レスポンス取り扱い（保存/表示）」で分解し、到達性境界が成立しているかを低侵襲に確定する
- PTES：
...
- MITRE ATT&CK：
  - 該当戦術（必要なら技術）：TA0007（Discovery）、TA0008（Lateral Movement）、TA0009（Collection）
  - 攻撃者の目的（この技術が支える意図）：内部サービス/メタデータの探索（Discovery）、内部への横移動補助（Lateral Movement）、機密情報（クラウド資格情報等）の収集（Collection）。Webhook SSRFは"外向き機能"を"内向き到達"に反転させる。
