...
- ASM/OSINT（DNS/TLS/HTTP）で得た外形情報を、Web検証で使える形（認証/認可/API/入力/設定の検証計画）に落とし込める。
...
## 推奨読書順序（最小）
1) `01_topics/01_asm-osint/03_http_観測（ヘッダ/挙動）と意味.md`
2) `01_topics/02_web/02_authn_00_認証・セッション・トークン.md`
3) `01_topics/02_web/03_authz_00_認可（IDOR BOLA BFLA）境界モデル化.md`
4) `01_topics/02_web/04_api_00_権限伝播・入力・バックエンド連携.md`
...
## 優先度基準（P0/P1/P2）
- P0：認証/認可/入口の境界が未確定で、以後の検証が全てズレる状態
- P1：境界は見えるが、例外パスや外部依存が多く、影響範囲が読めない状態
- P2：境界は概ね整理できており、検証の深掘り順の問題に落ちる状態
...
- WSTG：
  - 該当カテゴリ/テスト観点：Information Gathering をWeb検証実務に変換する段（入口確定→認証→認可→APIへ繋ぐ）。
