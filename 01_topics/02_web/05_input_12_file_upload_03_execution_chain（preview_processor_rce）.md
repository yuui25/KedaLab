...
- 想定する環境（例：クラウド/オンプレ、CDN/WAF有無、SSO/MFA有無）：
  - アップロード機能を持つWebアプリ
  - 処理チェーン（同期/非同期、ワーカー、外部SaaS、ジョブキュー）が存在
  - 各種処理器（Image/PDF/Office/Video/Archive）が存在
- できること/やらないこと（安全に検証する範囲）：
  - できること：危険な実行連鎖が存在することを観測差分で固める
...
- 扱う範囲（本ファイルの守備範囲）
  ...
  - 扱わない（別ユニットへ接続）：
    - 展開系のFS越境（zip/tar/7z） → `05_input_11_path_traversal_03_archive（zip_slip）.md`
    - SSRF詳細 → `05_input_09_ssrf_*`
    - XSS詳細 → `05_input_06_xss_*`
...
### 何が"確定"できるか：
- 処理チェーンの存在（アップロード直後/閲覧時/共有時/バッチのどれで処理が走るか）
- 処理器の種類（画像/PDF/Office/アーカイブ/動画等）
- 境界の弱点（ネットワーク到達性、権限分離、サンドボックス、リソース制限）

### 何が"推定"できるか（推定の根拠/前提）：
- 危険な実行連鎖の可能性（RCE/SSRF/OOB/DoS/情報漏えい）
- 影響範囲（ワーカー内に閉じるか、内部横展開できるか）

### 何は"言えない"か（不足情報・観測限界）：
- 具体的な脆弱性の存在（処理器の特定と詳細検証が必要）
- 影響の確定（成立条件の詳細確認が必要）

### よくある状態パターン（正常/異常/境界がズレている等）：
- パターンA：処理チェーンが存在し、サンドボックス/権限分離が弱い → 高リスク
- パターンB：処理チェーンが存在し、サンドボックス/権限分離が強い → 中リスク
- パターンC：処理チェーンが存在しない、または最小限 → 低リスク
...
## 次に試すこと（仮説A/Bの分岐と検証）
### 仮説A：処理チェーンが存在し、サンドボックス/権限分離が強い
- 観測：
  - ワーカーが別コンテナ/別VM/別VPCで動いている
  - ネットワーク到達性が制限されている
  - リソース制限がある
- 次の一手：
  - 処理器の特定と既知脆弱性の確認
...
### 仮説B：処理チェーンが存在し、サンドボックス/権限分離が弱い
- 観測：
  - ワーカーが内部ネットワークへ到達できる
  - ワーカー権限が広い
  - リソース制限がない
- 次の一手：
  - 影響評価（到達可能資産、権限範囲）を優先
  - 境界条件を原因として指摘し、アーキ改善を提案

## 手を動かす検証（Labs連動：観測点を明確に）
- 検証環境（関連する `04_labs/`）
  - 追加候補Lab（例）
    - `04_labs/02_web/05_input/12_file_upload_execution_chain/`
- 取得する証跡（目的ベースで最小限）：
  - アプリログ（file_id, trigger, processor, exec_env, network_access, runtime_ms）
  - HTTPリクエスト/レスポンス（Proxyログ/HAR）
- 観測の取り方（どの視点で差分を見るか）：
  - アップロード直後に生成されるか vs 閲覧時に生成されるか（時間差・リクエスト増加）
  - ファイル種別を変える（画像/PDF/Office/動画/アーカイブ）で生成物や時間差を観測
- 実施方法（最高に具体的）：観測の準備と相関キー
  - 証跡ディレクトリ（必須）
    ~~~~
    mkdir -p ~/keda_evidence/file_upload_execution_chain 2>/dev/null
    cd ~/keda_evidence/file_upload_execution_chain
    ~~~~
  - 検証の前提を固定（スコープ事故を防ぐ）
    - 必須で決める（レポート先頭に書く）
      - 対象は **許可されたスコープ** のみ
      ...
      - 破壊的な処理の実行は行わない
  - 相関キー（最低限）を作る（後で必ず効く）
    - request_id、user_id、file_id、trigger、processor、exec_env、network_access、runtime_ms
...
- この例で観測していること：
  - プレビュー生成が非同期かどうか、別APIがあるかを確認
  - 処理チェーンの入口（アップロード境界か、閲覧境界か、共有境界か）を特定
- 出力のどこを見るか（注目点）：
  - preview_status の有無（非同期処理の存在）
  - 別API（/preview, /thumbnail, /jobs）の呼び出し（ワーカーの存在）
  - 生成完了までの時間差、ポーリング有無
