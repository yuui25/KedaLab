...
- 想定する環境：CDN/WAF配下、マイクロサービス/API、SSO/MFA、SaaS連携が一般的。
- できること/やらないこと（安全に検証する範囲）：大量アクセスや負荷試験を前提にしない（観測中心）。
- 依存する前提知識（必要最小限）：DNS/TLS観測と突き合わせて境界を固める。
  - `01_topics/01_asm-osint/01_dns_委譲・境界・解釈.md`
  - `01_topics/01_asm-osint/02_tls_証明書・CT・外部依存推定.md`
- 扱う範囲（本ファイルの守備範囲）
  - 扱う：HTTPの観測と解釈、境界（資産/信頼/権限）・外部依存（CDN/WAF/SaaS）・成立条件の特定
  - 扱わない（別ユニットへ接続）
    - DNS観測 → `01_dns_委譲・境界・解釈.md`
    - TLS観測 → `02_tls_証明書・CT・外部依存推定.md`
    - JS観測 → `04_js_フロント由来の攻撃面抽出.md`
    - 認証/認可の深掘り → `01_topics/02_web/02_authn_00_認証・セッション・トークン.md`、`01_topics/02_web/03_authz_00_認可（IDOR BOLA BFLA）境界モデル化.md`
...
- よくある状態パターン（正常/異常/境界がズレている等）
  - パターンA：リダイレクトが複雑 → 認証境界や複数ドメイン運用（境界が多い）
  - パターンB：CORSが露出 → API境界の存在（ブラウザ利用の前提がある）
  - パターンC：キャッシュ関連ヘッダが強い → CDN配下の可能性（観測点や再現条件が重要）
...
- 代表的な攻め筋（この観測から自然に繋がるもの）
  - 攻め筋1：CDN/WAF終端の特定 → DNS/TLSと突き合わせ、外部終端の可能性を補強し、“同一リクエストを条件差だけ変えて”観測し、差分が生じる条件を特定する
  - 攻め筋2：SSO/認証境界の特定 → 認証後観測（許可範囲で）に切り替える前提を作り、リダイレクト先ドメイン/パラメータのパターンを整理し、境界モデルを作る
  - 攻め筋3：API境界の特定 → CORS応答、Content-Type、エラー応答の差分から、APIの想定利用形態を推定し、認証後観測（Proxy）で、実際のAPI呼び出しをトレースして境界を固める
...
- 実施方法（最高に具体的）：観測の準備と相関キー
  - 証跡ディレクトリ（必須）
    ~~~~
    mkdir -p ~/keda_evidence/http_03 2>/dev/null
    cd ~/keda_evidence/http_03
    ~~~~
  - 検証の前提を固定（スコープ事故を防ぐ）
    - 必須で決める（レポート先頭に書く）
      - 対象URLは **許可されたスコープ** のみ（契約範囲/社内許可範囲/自前検証環境）
      - 観測は **代表点の抽出** のみ（大量アクセス・負荷試験は前提にしない）
      - 観測視点（端末/経路/ネットワーク/条件差）を記録
  - 相関キー（最低限）を作る（後で必ず効く）
    - URL：対象URL（パス含む）
    - Method：HTTPメソッド（GET/POST等）
    - StatusCode：HTTPステータスコード
    - Headers：主要ヘッダ（Location、Set-Cookie、CORS、Cache-Control等）
    - Condition：観測条件（User-Agent、Accept、Cookie有無、参照元等）
...
- ASVS：
  ...
  - このファイルの内容が「満たす/破れる」ポイント：
    - 破れる：境界と依存を誤ると、ASVS観点の当て方（どこを見るべきか）がズレる。外部依存（CDN/WAF/SaaS/委託）を把握できないと、以降の検証で“対象外”や“前提違い”を起こし、検証精度が落ちる。
...
- PTES：
  ...
  - 前後フェーズとの繋がり（1行）：HTTP観測で入口と境界を固めることで、深掘り（認証/認可/API/設定）の検証を外さない
- MITRE ATT&CK：
