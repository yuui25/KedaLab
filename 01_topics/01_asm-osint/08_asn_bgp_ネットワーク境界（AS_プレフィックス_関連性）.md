...
## 前提（対象・範囲・想定）
- 対象：06_subdomain 等で得た FQDN→IP（A/AAAA）、既知の公開IP（ゲートウェイ、VPN、メール、CDN、API、管理画面等）
- 想定する環境：
...
- 扱う範囲（本ファイルの守備範囲）
  - 扱う：ASN/BGPによるネットワーク境界（自社/委託/クラウド/CDN）の推定、運用クラスターの特定、第三者境界の特定
  - 扱わない（別ユニットへ接続）
...
- 観測対象（ASN/BGPで見ているもの）
  ...
  - IP→ASN（Origin AS）を引く（ネットワーク境界の核）
    - このIPは、どのASが公告しているか（Origin AS）
    - ここがズレると意味が変わる差分：
      - 同一FQDNでも、解決先IPが複数ASに分散（マルチCDN/マルチクラウド/地域分散）
      - 同一ASでも、異なるサービス（CDN配下の別顧客）である可能性（断定しない）
  - ASN→公告プレフィックスを引く（範囲の“外形”を取る）
    - このASがどのIP範囲を公告しているか
  ...
  - 関連性（AS同士の関係）を“推定”に使う
    - 上流（transit）/下流（customer）/ピア（peering）の匂い
    - 特定ベンダのネットワークへの依存（第三者境界の強いシグナル）
    - 企業買収や事業分離の名残（別ASが併存する等）
- 境界の観点
  - 資産境界（管理主体・委託先・対象範囲の線引き）：「対象の運用ネットワーク」がどのAS/プレフィックス群に見えるか。Web入口（FQDN）が、どのネットワーク境界へ対応しているか。
  - 信頼境界（外部連携・第三者・越境ポイント）：CDN/WAF/クラウド/ホスティングへ越境している箇所（ASの分散として見える）。第三者運用の可能性が高いクラスター（報告/責任分界の材料）。
  - 権限境界（権限の切替/伝播/委任）（到達性境界として扱う）：“インターネット到達面”のネットワーク範囲がどこまでか。管理系（VPN/踏み台/管理UI）が別ASや別プレフィックスに分離されているか（設計の匂い）。
...
- 検証環境（関連する `04_labs/`）
  - 参照ファイル：
    ...
    - `04_labs/01_local/03_capture_証跡取得（pcap/har/log）.md`（到達性確認の最小証跡）
- 取得する証跡（目的ベースで最小限）
  - 入力（観測対象）：
...
- 実施方法（最高に具体的）：観測の準備と相関キー
  - 証跡ディレクトリ（必須）
    ~~~~
    mkdir -p ~/keda_evidence/asn_08 2>/dev/null
    cd ~/keda_evidence/asn_08
    ~~~~
  - 検証の前提を固定（スコープ事故を防ぐ）
    - 必須で決める（レポート先頭に書く）
      - 対象IPは **許可されたスコープ** のみ（契約範囲/社内許可範囲/自前検証環境）
      - 観測は **代表点の抽出** のみ（範囲外のスキャン拡大は前提にしない）
      - 観測視点（端末/経路/ネットワーク）を記録
  - 相関キー（最低限）を作る（後で必ず効く）
    - FQDN：対象FQDN
    - IP：解決されたIPアドレス（A/AAAA）
    - OriginAS：Origin AS（公告元）
    - ASName：AS名
    - Prefix：公告プレフィックス（外形）
    - Cluster：運用クラスター（同一/分散）
...
- ASVS：
  - 該当領域/章：V1（アーキテクチャ/境界の把握）とV14（運用・構成）を検証する“前提”として、インターネット到達面のネットワーク境界（自社/委託/クラウド/CDN）を固定する
  - 該当要件（可能ならID）：外部依存・設定・通信・境界（ASM/OSINTは“前提の崩れ”を潰す役）
  - このファイルの内容が「満たす/破れる」ポイント：
    - 破れる：ネットワーク境界（自社/委託/クラウド/CDN）を確定できないと、以降の検証で“対象外”や“前提違い”を起こし、検証精度が落ちる。
    - 満たす：外部依存・設定・通信・境界（ASM/OSINTは“前提の崩れ”を潰す役）を把握し、以降の検証（認証/認可/API/運用）を「前提崩れ」なく進める土台を作る。
- WSTG：
  - 該当カテゴリ/テスト観点：Information Gathering の一部として、到達性のあるホスト群を「ネットワーク境界」で束ね、Web側の入口（ホスト/サブドメイン）との対応を取る
  - 該当が薄い場合：この技術が支える前提（情報収集/境界特定/到達性推定 等）：WSTG各観点へ入る前の“入口確定”として接続
- PTES：
  - 該当フェーズ：Intelligence Gathering（対象のインフラ境界の確定）→ Vulnerability Analysis（どの範囲を深掘りすべきか）へ繋げる中核
  - 前後フェーズとの繋がり（1行）：ネットワーク境界を確定し、次の調査（PassiveDNS/CT/Cloud推定/スキャン範囲）へ迷いなく繋げる
- MITRE ATT&CK：
  - 該当戦術（必要なら技術）：Reconnaissance（T1590 Gather Victim Network Information / T1595 Active Scanning）
