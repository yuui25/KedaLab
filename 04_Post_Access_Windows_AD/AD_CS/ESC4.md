...
- **テンプレートの変更は CA 全体に影響する**：変更したテンプレートを使っている他のユーザー・サービスの証明書申請挙動が変わる可能性がある。本番環境では影響調査が必要
...
- **原状回復必須項目**: ✅ テンプレートを元の設定に戻す（Step 3 の `-configuration` で復元）/ ✅ 発行した証明書を CA で失効 / ✅ pfx・NT ハッシュ・TGT の暗号化保管・テスト完了時破棄
- **取得情報の取扱**: pfx ファイル・NT ハッシュ・TGT は暗号化保管、テスト完了後破棄
