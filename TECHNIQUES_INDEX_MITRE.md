...
| Technique ID | 技術名 | kedalab ファイル |
|------|-------|--------|
| T1190 | Exploit Public-Facing Application | `02_Initial_Access/Web_Vulnerabilities/*` / `02_Initial_Access/Edge_Appliance_CVEs.md` / `02_Initial_Access/Samba_Exploitation.md`（CVE-2007-2447 usermap script / CVE-2017-7494 SambaCry の未認証 RCE） / `02_Initial_Access/distcc_Exploitation.md`（CVE-2004-2687 distccd 未認証コマンド実行） / `02_Initial_Access/MySQL_Exploitation.md`（§10 CVE-2012-2122 認証バイパス） / `02_Initial_Access/PostgreSQL_Exploitation.md`（§8 COPY FROM PROGRAM CVE-2019-9193 経路 / §11 CVE-2018-1058 search_path schema poisoning） / `02_Initial_Access/Shellshock.md`（CVE-2014-6271 bash 環境変数インジェクション・CGI 経由 RCE） |
| T1133 | External Remote Services | `02_Initial_Access/SSH.md` / `02_Initial_Access/FTP.md` / `02_Initial_Access/Mail_Services.md` / `02_Initial_Access/WinRM.md` / `02_Initial_Access/Impacket_Exec.md` / `02_Initial_Access/Edge_Appliance_CVEs.md` / `02_Initial_Access/RDP.md` / `02_Initial_Access/MySQL_Exploitation.md`（外部公開 3306 への認証突破） / `02_Initial_Access/PostgreSQL_Exploitation.md`（外部公開 5432 への認証突破・listen_addresses 設定不備） |
| T1078.001 | Valid Accounts: Default Accounts | `02_Initial_Access/Default_Credentials.md` / `02_Initial_Access/MySQL_Exploitation.md`（§2 root 空パスワード・anonymous ユーザー） / `02_Initial_Access/PostgreSQL_Exploitation.md`（§2 postgres/postgres・pg_hba.conf の trust 認証） |
...
| T1059.003 | Command and Scripting Interpreter: Windows Command Shell | `04_Post_Access_Windows_AD/Enumeration_Checklist.md` / `02_Initial_Access/MySQL_Exploitation.md`（§8 UDF RCE Windows 経路・lib_mysqludf_sys.dll） / `02_Initial_Access/PostgreSQL_Exploitation.md`（§8 COPY FROM PROGRAM Windows 経路・powershell.exe 起動） |
| T1059.004 | Command and Scripting Interpreter: Unix Shell | `03_Post_Access_Linux/Shell_Stabilization.md` / `02_Initial_Access/MySQL_Exploitation.md`（§8 UDF RCE Linux 経路・lib_mysqludf_sys.so） / `02_Initial_Access/PostgreSQL_Exploitation.md`（§8 COPY FROM PROGRAM Linux 経路・sh -c で子プロセス起動・§9 PL/PerlU sys_exec） |
| T1059.006 | Command and Scripting Interpreter: Python | `02_Initial_Access/Web_Vulnerabilities/Command_Injection.md` / `02_Initial_Access/PostgreSQL_Exploitation.md`（§9 PL/PythonU 経由 UDF RCE・plpython3u extension・subprocess.check_output） |
...
| T1098.004 | Account Manipulation: SSH Authorized Keys | `02_Initial_Access/SSH.md`（§11 authorized_keys 書込・FTP / Redis / PostgreSQL / MySQL 各書込経路 + persistence） / `02_Initial_Access/MySQL_Exploitation.md`（§9 INTO OUTFILE 経由・`mysql:mysql` 所有者罠あり） / `02_Initial_Access/PostgreSQL_Exploitation.md`（§10 lo_export / COPY TO / COPY ... FROM PROGRAM chown 経由・`postgres:postgres` 所有者罠あり） |
...
| T1110.003 | Brute Force: Password Spraying | `05_Tools_Reference/Netexec.md` / `02_Initial_Access/Account_Lockout_Recon.md` / `02_Initial_Access/WinRM.md`（§6 nxc winrm --continue-on-success） / `02_Initial_Access/Impacket_Exec.md`（§8 nxc smb --continue-on-success → wmiexec 連続実行・ローカル管理者 hash 使い回し検出 with `--local-auth`） / `02_Initial_Access/RDP.md`（§6 nxc rdp --continue-on-success・targets.txt で cred reuse 検出） / `02_Initial_Access/MySQL_Exploitation.md`（§5 nxc mysql --continue-on-success・targets.txt で cred reuse 検出） / `02_Initial_Access/PostgreSQL_Exploitation.md`（§5 hydra postgres / medusa / nmap pgsql-brute・複数ホスト cred 使い回し検出） |
| T1110.001 | Brute Force: Password Guessing | `02_Initial_Access/Default_Credentials.md`（hydra / medusa） / `01_Reconnaissance/SNMP_Enumeration.md`（コミュニティ文字列ブルート） / `02_Initial_Access/RDP.md`（§6 crowbar / hydra rdp / ncrack） / `02_Initial_Access/MySQL_Exploitation.md`（§5 hydra mysql / medusa / ncrack / nmap mysql-brute） / `02_Initial_Access/PostgreSQL_Exploitation.md`（§5 hydra postgres / medusa / nmap pgsql-brute） |
...
| T1552.001 | Unsecured Credentials: Credentials In Files | `02_Initial_Access/Credential_Discovery.md`（.env / スクリプト埋め込み） / `02_Initial_Access/MySQL_Exploitation.md`（§7 LOAD_FILE で /var/www/html/config.php / wp-config.php 等の設定ファイル読取） / `02_Initial_Access/PostgreSQL_Exploitation.md`（§7 pg_read_file で postgresql.conf / pg_hba.conf / アプリ設定ファイル読取） |
| T1552.004 | Unsecured Credentials: Private Keys | `02_Initial_Access/Credential_Discovery.md` / `02_Initial_Access/SSH.md`（§5 鍵接続 / §8 ssh2john パスフレーズクラック / §10 Debian PRNG 弱鍵） / `02_Initial_Access/MySQL_Exploitation.md`（§7 LOAD_FILE で /home/[USER]/.ssh/id_rsa 取得） / `02_Initial_Access/PostgreSQL_Exploitation.md`（§7 pg_read_file / lo_import で /var/lib/postgresql/.ssh/id_rsa 取得） |
...
| T1210 | Exploitation of Remote Services | `02_Initial_Access/SMB_Windows_Exploitation.md`（§2 MS17-010 EternalBlue CVE-2017-0143〜0148 / §3 MS08-067 CVE-2008-4250 — 未認証 SYSTEM RCE） / `02_Initial_Access/Samba_Exploitation.md`（§2 CVE-2007-2447 usermap script / §3 CVE-2017-7494 SambaCry） / `02_Initial_Access/RDP.md`（§9 BlueKeep CVE-2019-0708 / §10 DejaBlue CVE-2019-1181 系 / §11 PyRDP MitM） |
