...
`--nmap` は nmap が検出したすべてのサービス×バージョンに対してヒットする結果を一括表示するため、数十件以上出ることがある。以下の手順で優先度をつける：
...
**`--nmap` で 0 件 / ヒットが少なすぎる場合 — クエリが長すぎる問題:**

nmap の `-sV` 出力には製品名・版数・OS サフィックス（例: `Samba smbd 3.0.20-Debian`）が含まれ、`--nmap` はこの長い文字列をそのままクエリに使う。Exploit-DB の登録名と一致しない語が混じると 0 件になりやすい。**製品名＋メジャー.マイナーだけに緩めて手動で再検索する。**

```bash
# --nmap が 0 件または [i] 行だけで終わった場合の手動再検索例
searchsploit "samba 3.0"              # 製品名 + メジャー.マイナーのみ（OS サフィックス除去）
searchsploit vsftpd 2.3               # 同様
searchsploit distccd                  # バージョン無しで製品名のみ（version 非依存の CVE を拾う）
```
