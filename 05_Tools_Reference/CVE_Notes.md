...
> **スコープ**: 特定のソフトウェア × バージョン限定の具体的ペイロードを集約する参照集。手法クラスの「シグナル → 次の動作」は各手法ファイルを参照し、ペイロードの詳細が必要になったときにここを開く。

## 着火条件

- バージョン特定済みの CVE に対して実際のペイロードを確認したい
- `Edge_Appliance_CVEs.md` / `Searchsploit.md` で CVE が特定済みで、詳細手順が必要な場合
...
| 状況 | 対処 |
|------|------|
| PoC を実行しても応答がない / エラー | バージョンが対象外。`Edge_Appliance_CVEs.md` の「成功シグナル確認」フロー（ステージ 1→2→3）に戻る |
| nuclei はヒットするが手動 PoC が動かない | TLS バージョン依存 / ヘッダー順序の問題。`curl -v` でリクエスト差分を確認 |
| エクスプロイト実行後に装置が再起動 | **業務停止リスク発火**。即中断し対象組織の連絡先に報告 |
| CVE 番号でヒットするが対象バージョンと一致しない | NVD の CPE 欄で正確な影響範囲を確認。パッチ済みの可能性 |
