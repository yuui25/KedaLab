...
SaaS・クラウドWebアプリ・APIのみを対象とした診断スコープで使う。

> **ペネトレとしてWebシステムを起点にシェル取得を目指す場合は、このファイルではなく `Linux_Attack_Flow.md` の Step 2 から進めること。**
> このファイルはWeb診断スコープ（脆弱性の網羅的な洗い出し）専用。
>
> **本番の場合**：本フローに入る前にスコープ・実施可否・除外エンドポイント（決済・本番データ等）について事前合意を確認すること。

> **記録しながら進める**：確認した機能・脆弱性・当たりの入力点は控えながら進む。各 Step 末尾の「▶ 記録」が、控えるべき成果と、kedaweb の Workspace ＞ Worksheet（記入の「型」）の対応欄を示す。Web 診断スコープなので主に「[02-W] WEB 脆弱性」チェックリストと各メモ欄を使う。
...
> **▶ 記録:** 開いたポート・見つけたディレクトリ/vhost・トリアージで挙がった機微情報を控える。→ Worksheet「PORTS」欄・「[01] RECON」/「[02-W] WEB」のメモ欄。
...
> **▶ 記録:** 「どの機能 → どの脆弱性を確認したか／結果」を控える（未確認・要再訪も含む）。→ Worksheet「[02-W] WEB 脆弱性」チェックリストのメモ欄。

---

## Step 3：認証・認可の横断確認

認証・認可の確認は基本3層（＋ JWT 検出時のみ4層目）で行う。上から順に確認すること。
...
> **▶ 記録:** 特定したバージョンと該当 CVE を控える。→ Worksheet「[02] INITIAL ACCESS」の「既知 CVE 照合」メモ欄。
