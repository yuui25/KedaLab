...
- 前提ツール（最小限）：ブラウザ + Proxy（HAR）、（任意）`curl`。
  - 理由：反射/保存/エラー差分を最小回数で観測できる。
  - 代替：`Invoke-WebRequest` でも可。
- 参照すべきtopics：
...
- 実際の観測例：
  - マーカー例：`keda_probe_20250101_1015`

### Step 1：入力が“どこに出るか”を分類する（反射/保存/無反応/エラー / 目安: 8分）
...
- 実際の観測例：
  - レスポンス本文に `keda_probe_...` が出る

### Step 2A：分岐A（反射/保存：解釈境界の候補 / 目安: 8分）
...
### Step 2B：分岐B（エラー手掛かり：実行境界の候補 / 目安: 8分）
...
### Step 2C：分岐C（外部連携/URL入力：SSRF/到達性境界 / 目安: 8分）
...
### Step 4：影響範囲（権限・環境・到達性）を整理する（目安: 5分）
...
## よくある失敗と対処法
- いきなり強い入力を入れる → 無害な差分から始める
- 入力点を増やしすぎる → 最大3点に絞る
- 反射と実行を混同する → まず「どこに出るか」を確定

## バグバウンティでの注意点
- 影響が大きい試行は避け、観測で成立条件を示す
- 外部到達性の確認は許可範囲内でのみ
- レポートは「最小差分」と「再現性」を明記
## 取得する証跡（目的ベースで最小限）
