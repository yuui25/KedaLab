...
> 原理（なぜ「列挙」は badPwdCount を増やさず、「認証試行」だけがロックアウトを発動させるか・SAMR / LSAT の読み取りメソッドと DCERPC BIND の境界）→ [`../06_Concepts/RPC_Enumeration_Internals.md`](../06_Concepts/RPC_Enumeration_Internals.md) §5
...
#   実値は WRITING_GUIDE.md の禁止例に該当しないものをテストごとに決める）
...
- **観察試行で本物のユーザーアカウントを使わない**。事前合意されたテスト用アカウント（例：`[テスト略号]_TEST`）で観察し、本物のユーザーへのスプレーは設計確定後に行う
