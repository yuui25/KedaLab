...
> **スコープ**: ネットワーク機器・ミドルウェア・管理コンソール・組込み機器の **製品出荷時のデフォルト認証情報** が運用後も変更されずに残っているケースを狙う手順を集約する。製品特定は `../01_Reconnaissance/` 配下で済んでいる前提。CVE 探索よりデフォルト試行を先に行うかの判断ガイドも含む。
...
| サーブレットコンテナ管理画面 | Tomcat manager / Tomcat host-manager | `/manager/html` で 401 + `Server: Apache-Coyote` | `tomcat/tomcat` / `tomcat/s3cret`（初期 `tomcat-users.xml` テンプレの代表例）/ `admin/admin` / `manager/manager` / `admin/[空]` |
...
| CI / CD | Jenkins / GitLab Omnibus | `/login` に Jenkins ロゴ、`X-Jenkins:` ヘッダー | Jenkins: **初期セットアップ（Setup Wizard）が未完了の場合**、`/script` が無認証で叩けることがある（初期パスワードは `${JENKINS_HOME}/secrets/initialAdminPassword` にローカル保存のみで、外部からデフォルト認証情報では入れない設計）。認証後は `/script` で Groovy RCE。/ GitLab: `root/5iveL!fe`（旧版） |
...
| データベース | MSSQL / MySQL / PostgreSQL / MongoDB / Redis / ElasticSearch | 1433 / 3306 / 5432 / 27017 / 6379 / 9200 各ポート | MSSQL: `sa/[空]` / `sa/sa` / MySQL: `root/[空]` / `root/root` / PostgreSQL: `postgres/postgres` または **`pg_hba.conf` が `trust` 認証の場合はパスワード不要で接続成立**（`psql -h [TARGET] -U postgres` で直接入れる）/ MongoDB: 認証無し（27017 直結）/ Redis: 認証無し（6379 直結 → `INFO`）/ ElasticSearch: 認証無し（旧版） |
...
| IP カメラ・NVR | Hikvision / Dahua / Axis / 各 OEM | `554/tcp` RTSP 開放、`80` で HTML title が `WEB SERVICE` 等 | Hikvision: `admin/12345`（**古典。2015 年以降の機種はファームウェアで初回パスワード変更を強制するよう変更されており通らないことが多い**）/ Dahua: `admin/admin` / Axis: `root/pass`（初期化要）/ ONVIF: `admin/[空]` |
...
## 1. 製品特定済みの状態で「最初の 1 発」を `curl` で当てる

辞書攻撃を回す前に **最も可能性の高い 1 ～ 3 組合せだけ手で試す**。それで通れば辞書攻撃は不要、通らなければ §2 以降の自動化に進む。
...
| 観測される出力 | 示唆 | 次のアクション |
|------------|-----|------------|
| `admin/admin` で初回ログイン後にパスワード変更画面 | 健全な初期セットアップフロー。認証情報は通っている | §6 の認証通過後アクションを確認 |
| ログイン成功後にそのまま管理機能が使える | 出荷状態のまま放置 | 管理機能の中で何ができるかを列挙（ファイル読込・コマンド実行）|
| `admin/admin` 不可だが製品由来組合せが通る | 出荷時候補を変更しただけ | `[製品名]/[製品名]123` 等を追加で試す |
| 全組合せが失敗 | 変更済み / IP 制限あり | §2 hydra または §4 nuclei に切替 |
...
| 出力 | 示唆 | 次のアクション |
|------|------|--------------|
| `[22][ssh] host: ... login: admin password: admin` | SSH 認証成功 | SSH でシェル取得 → `./SSH.md` |
| hydra が全件失敗 | デフォルト認証情報は変更済み | §4 nuclei で一括確認するか CVE 経路に切替 |
| 数回後に接続切断 / タイムアウト多発 | fail2ban / tarpit が発動 | `-W 5` で 5 秒待機、または試行間隔を広げる |

---

## 3. `nxc` で SMB / WinRM / MSSQL の管理者デフォルト
...
## 5. `nuclei` のデフォルト認証情報テンプレート（一括チェック）
...
- 後：WinRM に通った認証情報での本格的アクセス → `./WinRM.md`
