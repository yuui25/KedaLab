...
**着火条件:** §2 で匿名アクセスが成立、または認証情報で接続できた。匿名で書込可能なディレクトリがあれば認証なし RCE 経路の起点になる。
...
- 関連：FTP で取得した cred の他プロトコル使い回し → `SSH.md` / `Mail_Services.md` / `WinRM.md` / `Protocol_Exploitation.md`（Impacket exec / RPC 各セクション）
