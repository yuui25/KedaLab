...
**`strings` で取れない場合（難読化・パック済み）：**

- **FLOSS**（FireEye/Mandiant 製 OSS ツール、`pip install flare-floss`）を使うと、XOR スタック文字列など動的に生成される難読化文字列を静的に抽出できる。`strings` より遥かに高い捕捉率になる
- C/C++ / Go / Rust 等のネイティブバイナリで逆アセンブルが必要な場合は **Ghidra**（NSA 製 OSS）または **IDA**（商用）で解析する。本ファイルはスコープ外（静的解析に留める）
