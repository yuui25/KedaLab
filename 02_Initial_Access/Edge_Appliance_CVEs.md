...
> **スコープ**: インターネット境界に置かれるベンダー製アプライアンス（SSL-VPN ゲートウェイ / 次世代ファイアウォール / ロードバランサー）に対して、**「製品が判明したらまず照合する既知 CVE」** をベンダー別に集約する。CVE 個別の長文 PoC・ペイロード・バージョン対応の細部は `../05_Tools_Reference/CVE_Notes.md` 側に置き、本ファイルは「ベンダー → 該当 CVE → CVE_Notes へ」の遷移表に徹する。対象ベンダー: Citrix / Fortinet / Ivanti / Palo Alto / F5。
...
- 必要なツール：
  ...
  - **外部リソース依存：** Shodan / FOFA / Censys の Web UI / API（製品分布調査・フィンガープリント確認）。**事前合意の対象（オフサイト OSINT）** のため、スコープによっては使えない
...
- **そのアプライアンスが「本物の本番境界デバイス」か「ステージング / DMZ 上のテスト機」か**。本番にいる場合、PoC が業務停止を引き起こすと対象組織全体に影響する。事前合意で「対象がどの装置か」「業務時間外の試行可否」を必ず取り直す
...
## 3. 照合表のリンクから CVE_Notes の該当セクションへ
...
| バージョン文字列が `OEM` / カスタムビルドで一致しない | キャリア / OEM 提供のカスタム FW | 公開バージョン番号と OEM ビルド番号の対応表をベンダーサポートに確認。なければ Step 2 の nuclei 検知系のみで判断 |
...
> **[HIGH IMPACT]** エッジアプライアンス系 CVE の悪用は以下の理由で本番では原則禁止または個別合意必須：
...
- **国家攻撃者が先行利用する CVE が多い。** 過去事例として CVE-2024-3400 / CVE-2024-21887 等は **公開 PoC より先に IoC が公開される** ことがある。検査前にベンダー / CISA の最新アドバイザリで「侵害指標が既に出ていないか」を確認し、既に痕跡があれば対象組織にエスカレーション
- **検知系のみ実行する場合でも、対象側 WAF / IPS のアラート / 自動ブロックが発火する。** 検査用 IP がブロックされて以降の作業ができなくなる、または対象組織 SOC のインシデント対応コストが発生する。事前合意で「SOC への事前通知 / 検査時間枠」を必ず合意
...
- ✅ 取得したセッショントークン（Citrix Bleed 等）の破棄 → SIEM 側でセッション無効化を対象組織に依頼
- ✅ 試行ログ（テスター側 pcap / curl 履歴）の暗号化保管・テスト完了時破棄

```bash
# [Attacker] テスト識別子コメントマーカー方式で作業証跡を識別可能にする
...
- **原状回復必須項目**:
  ...
  - ✅ 取得したセッショントークン・認証情報の暗号化保管 → テスト完了時破棄
  - ✅ 装置側ログを対象組織に渡して「侵害指標と検査試行の区別」を支援
  - ✅ 試行内容（送信パケット / curl コマンド履歴 / nuclei 出力）の完全記録 → SOC への共有
- **取得情報の取扱**: 抽出した内部設定 / 認証情報 / セッショントークンはすべて暗号化保管。テスト完了時に破棄
