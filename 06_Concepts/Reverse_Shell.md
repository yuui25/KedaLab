...
本番のテストで使う場合は事前合意とリスニング側の隠蔽（HTTPSラッパー・C2フレームワーク経由）を前提にする。
...
> 特定のインターフェース名（VPN 専用 / 物理 LAN / コンテナ仮想 NIC 等）を決め打ちしない。
...
ポートは任意だが、本番のテストでは `443/tcp` `80/tcp` を選ぶ（egress フィルタを通りやすい）。
演習環境では 4444・1337・9001・8080 などがよく使われる。
ターゲットからの接続が来ると、そのままシェルとして操作できる。

### ③ Metasploit モジュールを使う場合は nc 不要

msf のリバースシェル系モジュールは **`run` と同時に内部でリスナーを自動起動する**。

> **同じ LPORT で nc を先に立てるとポート競合（`Handler failed to bind`）で msf が即中断する。**

| 経路 | リスナーの準備 |
|---|---|
| 手動 PoC / スクリプト / コマンドインジェクション | `nc -lvnp [PORT]` を自分で先に起動（上記②） |
| msf モジュール（reverse 系 payload） | **nc を立てない** — msf が `run` 時に自動起動 |

→ msf の設定・エラー読み分け詳細: `../05_Tools_Reference/Metasploit.md`
...
- **自分のIPを決め打ちしない**
  「VPN 経路なら VPN インターフェース」「物理 LAN なら物理 NIC」のような特定のインターフェース名前提の決め打ちはしない。
