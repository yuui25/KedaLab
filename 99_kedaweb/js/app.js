...
(() => {
  ...
  async function fetchMd(path) {
    // no-cache: revalidate every load. Local static servers (python -m http.server)
    // send no Cache-Control/ETag, so without this the browser heuristically serves
    // stale .md after edits even on hard reload (the click-time fetch isn't covered
    // by Ctrl+Shift+R's cache bypass).
  ...
  function wsBuildOutput(fmt) {
    ...
    for (const sec of (WS.sections || [])) {
      ...
      if (sec.type === "checklist") {
        ...
        if (sec.note) {
          const sn = (v[`secnote:${sec.id}`] || "").trim();
          if (sn) { L.push(""); L.push(md ? "_✎ 振り返り / 改善メモ:_" : "✎ 振り返り / 改善メモ:"); L.push(sn); }
        }
      } else {
    ...
    // Machine-readable state for lossless re-import (↑ 読み込み). In .md it lives in
    // an HTML comment so markdown viewers don't show it. JSON.stringify of the flat
    // {dataKey: value} map is single-line (user newlines escaped as \n).
    const stateJson = JSON.stringify(v);
    L.push("");
    L.push(md ? `<!-- KEDA_WS_STATE:${stateJson} -->` : `KEDA_WS_STATE:${stateJson}`);
    return L.join("\n");
  }
  ...
  // Restore form state from a previously exported .md/.txt by reading the embedded
  // KEDA_WS_STATE marker. Returns true on success, false if no valid marker found.
  function wsImportText(text) {
    const m = text.match(/KEDA_WS_STATE:(\{.*\})\s*(?:-->)?\s*$/m);
    if (!m) return false;
    let state;
    try { state = JSON.parse(m[1]); } catch (e) { return false; }
    if (!state || typeof state !== "object") return false;
    const form = document.getElementById("wsForm");
    if (!form) return false;
    // reset every field first so the imported file fully replaces current content
    $$("[data-k]", form).forEach(el => {
      if (el.classList.contains("ws-tri")) wsSetTri(el, "");
      else el.value = "";
    });
    wsApply(state);
    wsSave();
  ...
  function setupWorksheet() {
    ...
    const importBtn = document.getElementById("wsImport");
    const importFile = document.getElementById("wsImportFile");
    if (importBtn && importFile) {
      ...
      importFile.addEventListener("change", () => {
        ...
        reader.onload = () => {
          ...
          if (!/KEDA_WS_STATE:/.test(text)) {
            window.alert("読み込めませんでした。kedalab worksheet が書き出した .md / .txt（末尾に KEDA_WS_STATE 行があるもの）を選んでください。");
            importFile.value = "";
            return;
          }
          if (!window.confirm("現在の記入内容を、読み込むファイルの内容で置き換えます。よろしいですか？（先に .md/.txt で保存推奨）")) {
            importFile.value = "";
            return;
          }
          window.alert(wsImportText(text) ? "読み込みました。" : "読み込めませんでした（状態データが壊れています）。");
          importFile.value = "";  // allow re-selecting the same file
        };
        reader.onerror = () => { window.alert("ファイルの読み取りに失敗しました。"); importFile.value = ""; };
        reader.readAsText(f);
      });
    }
  }
